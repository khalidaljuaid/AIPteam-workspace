...
let ThemeModeComponentInstance: ThemeMode | null = null

const ThemeModeComponent = {
  getInstance: () => {
    if (typeof window === 'undefined') {
      // @ts-ignore - Mock object for SSR
      return {
        getMode: () => 'light' as Mode,
        setMode: () => { },
        getMenuMode: () => '' as Mode | '',
        getSystemMode: () => 'light' as Mode,
        flipImages: () => { },
        on: () => { },
        off: () => { },
        init: () => { },
    ...
    if (!ThemeModeComponentInstance) {
      ThemeModeComponentInstance = new ThemeMode()
    }
    return ThemeModeComponentInstance
  },
  getMode: function () { return this.getInstance().getMode() },
  setMode: function (mode: Mode, menuMode: Mode | '') { return this.getInstance().setMode(mode, menuMode) },
  getMenuMode: function () { return this.getInstance().getMenuMode() },
  getSystemMode: function () { return this.getInstance().getSystemMode() },
  flipImages: function () { return this.getInstance().flipImages() },
  on: function (name: string, handler: Function) { return this.getInstance().on(name, handler) },
  off: function (name: string, handlerId: string) { return this.getInstance().off(name, handlerId) },
  init: function () { return this.getInstance().init() },
}
